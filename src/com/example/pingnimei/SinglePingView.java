...
public class SinglePingView extends RelativeLayout {
	...
	public void updateView(PingMessage pingMsg) {
        ...
        if (distance != null) {
            BmobGeoPoint geoPoint = pingMsg.getLocation();
            BDLocation myLocation = LocationAction.getInstance().getLocation();
            double latitude = 0;
            double longitude = 0;
            if (myLocation != null
                    && (myLocation.getLocType() == BDLocation.TypeGpsLocation
                            || myLocation.getLocType() == BDLocation.TypeNetWorkLocation
                            || myLocation.getLocType() == BDLocation.TypeOffLineLocation)) {
                longitude = myLocation.getLongitude();
                latitude = myLocation.getLatitude();
            }
            distance.setText("与你的距离："
                    + gps2m(geoPoint.getLatitude(), geoPoint.getLongitude(),
                            latitude, longitude) + "米");
        }
	}
